...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	float area = grid[0].size() * grid.size();
	float cellValue = 1.0 / area;
...
// initializing newGrid
  vector < vector <float> > newGrid;
  vector <float> zeros (beliefs.size() , 0);

  for (int i = 0; i < beliefs[0].size(); i++) {
	  newGrid.push_back(zeros);
  }
  ...
  for (int i = 0; i < height; i++) {
	  for (int j = 0; j < width; j++) {
		  new_j = (i + dy + width) % width;
		  new_i = (j + dx + height) % height;
...
//				cout << "if " << newGrid[i][j] << endl;
				S = S + (beliefs[i][j] * p_hit);
			}
			else {
				newGrid[i][j] = (beliefs[i][j] * p_miss);
//				cout << "else " << newGrid[i][j] << endl;
				S = S + (beliefs[i][j] * p_miss);
			}

		}
//		newGrid.push_back(belief);
//		belief.clear();
	}
	/*
		for (int i = 0; i < beliefs.size(); i++) {
